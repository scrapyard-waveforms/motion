...
/**
 * Magnetometer on a MonochromePanel (SSD1306 / SH1106).
 *
 *   ./runner magnetometer-oled-demo lis3mdl
 *
 * Requires tubes.defaults.canvas → a MonochromePanel.
 */
#[SketchAttribute('magnetometer-oled-demo')]
class OLEDTestSketch extends Sketch
{
    ...
    protected string $description = 'Magnetometer X/Y/Z + magnitude on a MonochromePanel (Ctrl-C to stop)';
    ...
    public function loop(): SketchLoopResult
    {
        if ($this->stopRequested || $this->defaultCanvasShouldStop()) {
            $this->info('Magnetometer OLED demo stopped.');
        ...
        if (! $this->announced) {
            $this->info(
                "Magnetometer OLED via Magnetometer::circuit('{$this->circuitProfile}') → canvas [{$this->canvasProfile}]"
            );
